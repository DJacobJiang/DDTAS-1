...
def heat(x, log_dir, dataset, epoch):
    ...
    # xx = xx[0:100 ,0:100]
    # print('xx',xx)
    plt.matshow(xx, cmap='Blues', vmin=0, vmax=1)
    plt.colorbar()
    im_path = '%s/%s_%s_Heat.png' % (log_dir, dataset, epoch)
    plt.savefig(im_path,dpi =900)
    plt.close()

    plt.matshow(xx[0:500 ,0:500], cmap='Blues' , vmin=0, vmax=1)
    plt.colorbar()
    im_path = '%s/%s_%s_HeatL.png' % (log_dir, dataset, epoch)
    plt.savefig(im_path, dpi=900)
    plt.close()
